...
/*
	[알고리즘]
	입력으로 N이 주어졌을 때 실선으로 이루어진 도형의 둘레의 길이를 구하면 되는 문제이다. 이 문제의 패턴

	N = 1일 때:
	N = 1일 때의 둘레를 직접 세보면 4가 나온다.

	N = 2일 때:
	N = 2일 때의 둘레를 직접 세보면 8이 나온다.

	N = 3일 때:
	N = 3일 때의 둘레를 직접 세보면 12가 나온다.

 

	둘레의 길이는 4N이다. 
	따라서 입력을 받고 N에 4를 곱해준 값을 출력하면 된다. 
	단, 이 문제에서 N의 범위가 int의 범위를 초과하므로 long형을 써서 N을 입력받아야 한다. 
*/

public class step_10_04 {
